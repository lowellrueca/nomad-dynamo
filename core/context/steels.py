import clr
from ..db import get_element_types_of_family
from ..params import *
...
def sort_hss(family, base, depth, thickness):
    ...
    """
    element_types = get_element_types_of_family(family)
    sz = {'b': base, 'd': depth, 't': thickness}
    sort_by_size = lambda e: (get_length(e, sz['b']), get_length(e, sz['d']), get_length(e, sz['t']))

    # sort shs types
    shs_types = filter(lambda e: get_length(e, sz['b']) == get_length(e, sz['d']), element_types)
    shs_types_by_sz = sorted(shs_types, key=lambda e: sort_by_size(e)) # sort shs types by sizes

    # sort rhs types
    rhs_types = filter(lambda e: get_length(e, sz['b']) != get_length(e, sz['d']), element_types)
